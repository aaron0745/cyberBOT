...
that alternates the bot's activity text between two phrases, and --- most importantly --- starts a
single \textbf{30-second \texttt{setInterval}} that performs both scheduled posting and
anti-deletion recovery in one pass. There is only one interval; both jobs share it.
...
\begin{description}[leftmargin=0pt, itemsep=0.4em]
    ...
    description connection\_info start\_time end\_time}]]
    Updates any subset of the listed fields on an existing challenge and, if it is already live,
    edits the embed in place --- preserving the existing First Blood field rather than resetting
    it to "Waiting...".
