...
UnlockedHint   & (user\_id, hint\_id) compound unique index & Which players have unlocked which hints; records \texttt{cost\_paid} so refunds only reimburse points actually spent. \\[0.3cm]
...
role\_challenge\_ping        & Role ID pinged when a challenge is posted or scheduled drop occurs. \\
...
hints           & Lists every hint for a challenge; locked hints render a per-hint \texttt{buy\_hint} button showing its cost. On expired challenges or for hidden users, all hints are displayed unlocked and free immediately. \\
buy\_hint        & Deducts the hint's cost from the buyer's score (or 0 if expired), records an \texttt{UnlockedHint} with \texttt{cost\_paid}, and refreshes the leaderboard if points changed. Hidden users get the hint text for free with no record kept. \\
...
\begin{enumerate}
    ...
    \item \textbf{Championship handoff:} compares the top scorer's \texttt{user\_id} against
          \texttt{current\_champion\_id}. On a change, it updates the stored champion. If no champion
          previously held the belt, it broadcasts a "\textbf{FIRST BLOOD!}" belt claim alert in the
          \texttt{channel\_general} channel; if a reigning champion was overtaken, it broadcasts a
          "\textbf{NEW KING!}" alert naming both the new leader and who was dethroned. If configured,
          it strips \texttt{role\_champion} from every member currently holding it guild-wide and
          grants it to the new \#1.
\end{enumerate}
...
to a disabled "Time Expired" button, while the active Hints and Solves buttons remain available. Called after a correct submission, by hint-management commands, and by \cmd{/edit} to edit the live challenge post in place without creating new messages. If the message was deleted, recreation is left strictly to the background watchdog.
...
\begin{itemize}
    ...
    \item \textbf{Expired Challenge Free Hints:} When a challenge has expired (\texttt{end\_time} passed),
          all of its hints become completely free. Clicking the \textbf{Hints} button displays all clues
          immediately with no point deductions and no buy buttons. Any unlock recorded after expiry
          stores \texttt{cost\_paid: 0}.
    \item \textbf{Exact Refund Accounting:} \texttt{UnlockedHint} stores the exact \texttt{cost\_paid}
          at unlock time. When an administrator deletes a hint (\cmd{/remove\_hint}) or deletes a
          challenge (\cmd{/delete}), only users who actually spent points are refunded; users who unlocked
          clues for free after expiry receive no unearned points.
...
\begin{description}[leftmargin=0pt, itemsep=0.4em]
    ...
    Publishes an existing draft. Times use 24-hour \texttt{DD/MM HH:MM} format (e.g. \texttt{25/12 14:00}); a past \param{start\_time} posts
    ...
    edits the existing challenge card in place via \texttt{updateChallengePost()} (whether active or expired).
    It never creates a new card; if the message was deleted from Discord, recreation is handled strictly by the
    anti-deletion watchdog. It compares the new values with the old values and posts a detailed modification
    "diff" card to the configured \texttt{channel\_admin\_logs} channel.
    ...
    \item[\cmd{/list}]
    Ephemeral embed listing every challenge's name, points, and category.
...
\begin{description}[leftmargin=0pt, itemsep=0.4em]
    ...
    general\_channel champion\_role challenge\_ping\_role channel\_admin\_logs}]]
    Writes whichever of the seven optional options were supplied to their corresponding
    ...
    \item[\cmd{/redeploy}] \textbf{Logged.}
    Queries the GitHub API to check whether new commits exist compared to the running deployment.
    If new commits exist, triggers the Render Deploy Hook. If already up-to-date, displays an
    interactive prompt asking whether to restart the bot instead.

    \item[\cmd{/restart}] \textbf{Logged.}
    Gracefully exits the Node.js process (\texttt{process.exit(0)}). Render's supervisor automatically
    reboots the container within 5--10 seconds.
\end{description}
...
\begin{enumerate}
    ...
    \item If it changed: update \texttt{current\_champion\_id}. If there was no previous champion,
          broadcast a \textbf{FIRST BLOOD!} belt claim alert to the general channel; if a champion was
          dethroned, broadcast a \textbf{NEW KING!} alert naming both the new leader and who was dethroned.
          If \texttt{role\_champion} is configured, strip the champion role from every current holder
          guild-wide, and grant it to the new leader.
\end{enumerate}
...
/setup                 & Configure channels and roles (champion, challenge ping). \\
...
/redeploy             & Trigger Render rebuild via Deploy Hook (with commit check). \\
/restart              & Reboot bot process via Render supervisor. \\
